...
test('static hosting serves index and denies dotfiles with security headers', async (t) => {
    ...
    fs.writeFileSync(path.join(staticRoot, 'assets', 'generated', 'listening-exams', 'listening-practice-unified.html'), [
        '<!doctype html><html data-listening-wrapper="true"><head><title>Unified Listening</title></head>',
        '<body><div id="listening-wrapper-root" data-exam-id="" data-source-url="" data-base-href=""></div>',
        '<iframe id="listening-practice-frame"></iframe>',
        '<script src="/js/bundles/listening-wrapper.bundle.js" defer></script></body></html>'
    ].join(''), 'utf8');
    ...
    fs.writeFileSync(path.join(staticRoot, 'assets', 'generated', 'listening-exams', 'listening-index.compat.js'), [
        '(function registerListeningExamIndex(global) {',
        '  global.listeningExamIndex = [{',
        '    id: "listening-p1-sample",',
        '    type: "listening"',
        '  }];',
        '})(typeof window !== "undefined" ? window : globalThis);',
        ''
    ...
    try {
        ...
        assert.match(prettyListening.text, /Unified Listening/);
        assert.match(prettyListening.text, /id="listening-wrapper-root"/);
        assert.match(prettyListening.text, /data-exam-id="listening-p1-sample"/);
        assert.match(prettyListening.text, /data-source-url="\/ListeningPractice\/P1\/sample.html"/);
        assert.match(prettyListening.text, /data-base-href="\/ListeningPractice\/P1\/"/);
        assert.doesNotMatch(prettyListening.text, /Listening Sample/);
        const prettyListeningCsp = prettyListening.response.headers.get('content-security-policy') || '';
        assert.match(prettyListeningCsp, /script-src 'self'(?:;|$)/);
        ...
        assert.match(prettyListeningCsp, /frame-src 'self'/);
        assert.match(prettyListeningCsp, /child-src 'self'/);
        assert.doesNotMatch(prettyListeningCsp, /sandbox/);

        const shortPrettyListening = await client.request('GET', '/practice/listening/p1-high-007');
        assert.equal(shortPrettyListening.response.status, 200);
        assert.match(shortPrettyListening.text, /Unified Listening/);
        assert.match(shortPrettyListening.text, /data-exam-id="listening-p1-high-007-sample-title-aaaaaaaa"/);
        assert.match(shortPrettyListening.text, /data-source-url="\/ListeningPractice\/P1\/sample.html"/);
        assert.match(shortPrettyListening.text, /data-base-href="\/ListeningPractice\/P1\/"/);
        assert.doesNotMatch(shortPrettyListening.text, /Listening Sample/);

        const duplicateBaseListening = await client.request('GET', '/practice/listening/p2-low-023');
        assert.equal(duplicateBaseListening.response.status, 404);

        const duplicateHashListening = await client.request('GET', '/practice/listening/p2-low-023-22222222');
        assert.equal(duplicateHashListening.response.status, 200);
        assert.match(duplicateHashListening.text, /data-exam-id="listening-p2-low-023-stanley-island-22222222"/);
        assert.match(duplicateHashListening.text, /data-source-url="\/ListeningPractice\/vip%20special\/ListeningPractice\/P2\/nested\/nested.html"/);
        ...
        assert.match(nestedPrettyListening.text, /Unified Listening/);
        assert.match(nestedPrettyListening.text, /data-exam-id="listening-p2-nested"/);
        assert.match(nestedPrettyListening.text, /data-source-url="\/ListeningPractice\/vip%20special\/ListeningPractice\/P2\/nested\/nested.html"/);
        assert.match(nestedPrettyListening.text, /data-base-href="\/ListeningPractice\/vip%20special\/ListeningPractice\/P2\/nested\/"/);
        assert.doesNotMatch(nestedPrettyListening.text, /Nested Listening/);
